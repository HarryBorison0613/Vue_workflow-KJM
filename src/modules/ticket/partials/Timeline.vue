		...
		<ul class="-mb-8">
			<li v-for="(activityItem, activityItemIdx) in activity" :key="activityItem.id">
				<div class="relative pb-8">
					<span v-if="activityItemIdx !== activity.length - 1" aria-hidden="true" class="absolute top-5 left-5 -ml-px h-full w-0.5 bg-gray-200" />
					<div class="relative flex items-start space-x-3">
						<template v-if="activityItem.type === 'comment'">
							<div class="relative">
								<img :src="activityItem.imageUrl" alt="" class="h-10 w-10 rounded-full bg-gray-400 flex items-center justify-center ring-8 ring-white" />
								<span class="absolute -bottom-0.5 -right-1 bg-white rounded-tl">
									<ChatAltIcon aria-hidden="true" class="h-5 w-5 text-gray-400" />
								</span>
							</div>
							...
							<div>
								<div class="relative px-1">
									<div class="h-8 w-8 bg-gray-100 rounded-full ring-8 ring-white flex items-center justify-center">
										<UserCircleIcon aria-hidden="true" class="h-5 w-5 text-gray-500" />
									</div>
								</div>
							</div>
						...
						<template v-else-if="activityItem.type === 'tags'" condition="activityItem.type === 'tags'">
							<div>
								<div class="relative px-1">
									<div class="h-8 w-8 bg-gray-100 rounded-full ring-8 ring-white flex items-center justify-center">
										<TagIcon aria-hidden="true" class="h-5 w-5 text-gray-500" />
									</div>
								</div>
							</div>
